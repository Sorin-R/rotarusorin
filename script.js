...
const toggleSound = () => {
    ...
    buttons.forEach(btn => {
        ...
        if (icon) {
            // Synchronize all sound toggle icons across the page (Desktop & Mobile)
...
const bindClickEvents = idSoundDelayPairs => {
    idSoundDelayPairs.forEach(({ id, sound, delay }) => {
        ...
        elements.forEach(el => {
            el.addEventListener('click', () => playSound(sound, delay));
        });
    });
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateBurgerBackground() {
        ...
        if (burger)
            burger.style.backgroundColor = isActive ? '#f8f8f8' : currentColor;
        if (navList)
            navList.style.backgroundColor = isActive ? '#f8f8f8' : currentColor;
        if (hamburger) {
            hamburger.style.backgroundColor = isActive ? '#f8f8f8' : '';
        }
    }
    ...
    burgerIcon.addEventListener('click', function () {
        ...
        [hamburger, navList, socialNetwork]
            .filter(Boolean)
            .forEach(el => el.classList.add('active'));
        // Hide distracting background elements immediately
        [burgerIcon, delayedText, delayedText1, scrollCircle]
            .filter(Boolean)
    ...
    function closeMenu() {
        ...
        // Wait for the slide animation to finish before showing original elements
        setTimeout(() => {
            [burgerIcon, delayedText]
                .filter(Boolean)
                .forEach(el => (el.style.display = 'block'));
            [closeIcon, delayedText1]
                .filter(Boolean)
                .forEach(el => (el.style.display = 'none'));
        }, 600);
        updateBurgerBackground();
